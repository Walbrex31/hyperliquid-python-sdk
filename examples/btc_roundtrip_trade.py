...
def find_ubtc_usdc_pair(info):
    """Find the correct asset identifier for UBTC/USDC trading pair"""
    spot_meta = info.spot_meta()
    
    print("🔍 Looking for UBTC/USDC trading pair...")
    for asset_info in spot_meta["universe"]:
        ...
        if pair_name == "UBTC/USDC":
            print(f"✅ Found UBTC/USDC pair: {name}")
            return name, pair_name
    
    raise Exception("UBTC/USDC trading pair not found!")
...
def main():
    """
    Perform a single BTC/USDC round-trip trade:
    1. Buy BTC with USDC at market price
    2. Immediately sell all BTC back to USDC at market price
    3. Calculate P&L and fees paid
    """
    
    print("=" * 60)
    print("BTC/USDC ROUND-TRIP TRADE SIMULATOR")
    print("=" * 60)
    print()
    ...
    try:
        ...
        # Find UBTC/USDC trading pair
        ubtc_asset, pair_name = find_ubtc_usdc_pair(info)
        print()
        
        # Get current market price for UBTC
        all_mids = info.all_mids()
        ubtc_price = float(all_mids[ubtc_asset])
        print(f"📈 Current UBTC price: ${ubtc_price:,.2f}")
        print()
        ...
        initial_ubtc = get_balance(info, address, "UBTC")
        print(f"USDC: {initial_usdc:,.6f}")
        print(f"UBTC: {initial_ubtc:,.8f}")
        print()
        
        # Calculate trade size (use 100% of USDC for maximum volume)
        trade_usdc_amount = initial_usdc
        estimated_ubtc_amount = trade_usdc_amount / ubtc_price
        ...
        print(f"Estimated HYPE to buy: {estimated_hype_amount:.6f} HYPE")
        print(f"Max slippage: 0.1% (tight protection)")
        print()
        
        # Step 1: Buy HYPE with USDC (market order)
        print("🛒 STEP 1: BUYING HYPE")
        print("-" * 30)
        print(f"Placing market buy order for ~{estimated_hype_amount:.6f} HYPE...")
        
        buy_result = exchange.market_open(
            name=hype_asset,
            is_buy=True, 
            sz=estimated_hype_amount,
        ...
        hype_after_buy = get_balance(info, address, "HYPE")
        
        print(f"USDC after buy: {usdc_after_buy:,.6f}")
        print(f"HYPE after buy: {hype_after_buy:,.6f}")
        
        actual_hype_bought = hype_after_buy - initial_hype
        usdc_spent = initial_usdc - usdc_after_buy
        
        if actual_hype_bought <= 0:
            raise Exception("No HYPE was received from buy order!")
        
        print(f"✅ Actually bought: {actual_hype_bought:.6f} HYPE")
        print(f"✅ Actually spent: ${usdc_spent:.6f} USDC")
        print(f"✅ Effective buy price: ${usdc_spent/actual_hype_bought:.4f}")
        print()
        
        # Step 2: Sell all HYPE back to USDC (market order)
        print("💰 STEP 2: SELLING HYPE")
        print("-" * 30)
        print(f"Placing market sell order for {actual_hype_bought:.6f} HYPE...")
        
        sell_result = exchange.market_open(
            name=hype_asset,
            is_buy=False,
            sz=actual_hype_bought,
            slippage=0.001  # 0.1% slippage tolerance
        )
        ...
        final_hype = get_balance(info, address, "HYPE")
        
        print(f"USDC after sell: {final_usdc:,.6f}")
        print(f"HYPE after sell: {final_hype:,.6f}")
        
        usdc_received = final_usdc - usdc_after_buy
        hype_sold = hype_after_buy - final_hype
        
        print(f"✅ Actually sold: {hype_sold:.6f} HYPE")
        print(f"✅ Actually received: ${usdc_received:.6f} USDC")
        print(f"✅ Effective sell price: ${usdc_received/hype_sold:.4f}")
        print()
